...
class replay_buffer:
    ...
    def no_related_add(self, obs, action, reward, obs_, done, mu):
        data = (obs, action, reward, obs_, done, mu)
        if self.next_idx >= len(self.storge):
            self.storge.append(data)
        else:
            self.storge[self.next_idx] = data
    ...
    # sample from the memory
    def no_related_sample(self, batch_size):
        idxes = [random.randint(0, len(self.storge) - 1) for _ in range(batch_size)]
        return self._encode_no_related_sample(idxes)
    # encode samples
    def _encode_no_related_sample(self, idx):
        obses, actions, rewards, obses_, dones, mus = [], [], [], [], [], []
        for i in idx:
            data = self.storge[i]
            obs, action, reward, obs_, done, mu = data
            obses.append(np.array(obs, copy=False))
            actions.append(np.array(action, copy=False))
            rewards.append(reward)
            obses_.append(np.array(obs_, copy=False))
            dones.append(done)
            mus.append(np.array(mu, copy=False))
        return np.array(obses), np.array(actions), np.array(rewards), np.array(obses_), np.array(dones), np.array(mus)
